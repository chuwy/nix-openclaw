...
let
  ...
  npmHooksForNode = npmHooks.override { nodejs = nodejs_26; };
  ...
  pluginSrc =
    if (lock.tarballUrl or null) != null then
      ...
      stdenvNoCC.mkDerivation {
        ...
        installPhase = ''
          ${nodejs_26}/bin/node ${../scripts/openclaw-runtime-plugin-fetch-source.mjs} \
  ...
  drv = stdenvNoCC.mkDerivation (
    {
      ...
      postPatch = lib.optionalString isShrinkwrap ''
        ${nodejs_26}/bin/node ${../scripts/openclaw-runtime-plugin-prepare-npm.mjs}
      '';
      ...
      installPhase = "${nodejs_26}/bin/node ${../scripts/openclaw-runtime-plugin-install.mjs}";
